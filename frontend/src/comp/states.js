    ...
    return (
        <div className='states shadow-sm'>
            <div className='Top-states row border-bottom'>
               {loading? <h1 className='text-muted p-4 m-4'>Loading...</h1>: 
               
               <div className='revenue col-md-9'>
                    <div className='d-flex justify-content-between align-items-center '>
                    <h3>Revenue</h3>
                    <input type={'month'} className="date-input" value={date} onChange={(e)=> setdate(e.target.value)}/>
                    </div>
                    <h1 className='text-primary'>Rs. {total}</h1>
                   {data.length<1 ? <h1 className='text-center py-4 text-muted'>No date to show</h1>: <ResponsiveContainer width="100%" height={120}>
                    <BarChart
                      width={500}
                      height={300}
                      data={data.reverse()}
